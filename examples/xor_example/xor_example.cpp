...
void run(const std::string &config_file)
{
    ...
    Genome_ptr best_genome = p.run(eval_genomes, 2);

    std::cout << "\nPopulation:" << std::endl;
    for (std::pair<const int, Genome_ptr> &git : p.population)
    {
        Genome_ptr g = git.second;
        std::cout << "\nGenome:\n"
                  << g->key << std::endl;
        std::cout << "Key: " << g->key << std::endl;
        std::cout << "Fitness:\n"
                  << g->fitness << std::endl;
        std::cout << "Fitness: " << g->fitness << std::endl;
        std::cout << "Nodes:" << std::endl;
        for (std::pair<const int, NodeGene_ptr> &ngit : g->nodes)
        {
        ...
        for (std::pair<const std::pair<int, int>, ConnectionGene_ptr> &cgit : g->connections)
        {
    ...
    std::cout << "Key: " << best_genome->key << std::endl;
    std::cout << "Fitness: " << best_genome->fitness << std::endl;
    std::cout << "Nodes:" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    std::string fname = "examples/xor_example/config/xor_example.cfg";
